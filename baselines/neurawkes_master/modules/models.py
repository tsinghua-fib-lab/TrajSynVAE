...
class HawkesCTSM(object):
    ...
    def __init__(self, settings):
        ...
        if settings['path_pre_train'] == None:
            ...
            a = numpy.ones((self.dim_process,), dtype=dtype)*0.01
            self.mu = theano.shared(a, name='mu')
            '''
            ...
            '''
            a = numpy.ones((self.dim_process, self.dim_process), dtype=dtype)*0.01
            self.alpha = theano.shared(a, name='alpha')
            a = numpy.ones((self.dim_process, self.dim_process), dtype=dtype)
            self.delta = theano.shared(a, name='delta')
            #
        else:
